        ...
        function obj=draw(obj)
            ...
            for i=1:length(obj.RTick)
                obj.RLabelHdl(i)=text(obj.ax,cos(tnr).*(obj.RTick(i)-obj.RLim(1))./diff(obj.RLim),...
                                      ...
                                      sprintf('%.0f',obj.RTick(i)),'FontName','Arial','FontSize',12,'FontWeight','normal');
            end

            % 绘制属性标签
            for i=1:obj.PropNum
                obj.PropLabelHdl(i)=text(obj.ax,cos(tn(i)).*1.1,sin(tn(i)).*1.1,obj.PropName{i},...
                    'FontWeight','normal','FontSize',14,'HorizontalAlignment','center');
            end
...
% =========================================================================
        ...
        function obj=setType(obj,Type)
            ...
            if isequal(obj.Type,'Line')
                obj.LgdHdl=legend(obj.ax,[obj.PatchHdlL],obj.ClassName,'FontWeight','normal','FontSize',14,'Location','best','Tag','SRClagend');
            else
                obj.LgdHdl=legend(obj.ax,[obj.PatchHdlP],obj.ClassName,'FontWeight','normal','FontSize',14,'Location','best','Tag','SRClagend');
            end
        end
