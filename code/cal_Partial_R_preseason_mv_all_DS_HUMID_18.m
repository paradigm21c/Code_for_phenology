...
path_cal = sprintf('%s/Parks_polygon/ParkDS/HUMID/',path_home);

mkn = length(files_mask);

vin = {'EVI2','NDVI','NIRv'};

path_data = sprintf('%s/Parks_polygon/ParkDS/HUMID/',path_home);
...
for winsize = [7,8,9,10, 12, 15]
    percent_save_neg= nan(size(1:19-winsize,2),1);
    percent_save_pos= nan(size(1:19-winsize,2),1);

    for dayn =[1,2,4]
        for vi =1:length(vin)
            for phn=[1 4 5 7]
                for i = 1:19-winsize %[1,23-winsize]
                    % Tmax / Tmin
                    parfor ps = 0:120
                    ...
                    percent_pos=sum(pval_max(:,phn) < 0.05 & rho_max(:,phn) > 0,"all","omitmissing")./sum(~isnan(rho_max(:,phn)));
                    percent_neg=sum(pval_max(:,phn) < 0.05 & rho_max(:,phn) < 0,"all","omitmissing")./sum(~isnan(rho_max(:,phn)));
    ...
    save(sprintf('%s/Rho_%s_DS_mv%d_2018.mat',path_data,product,winsize),'rho_stack');
    save(sprintf('%s/Pval_%s_DS_mv%d_2018.mat',path_data,product,winsize),'pval_stack');
    save(sprintf('%s/PS_%s_DS_mv%d_2018.mat',path_data,product,winsize),'ps_stack');
    save(sprintf('%s/Pos_%s_DS_mv%d_2018.mat',path_data,product,winsize),'pos_stack');
    save(sprintf('%s/Neg_%s_DS_mv%d_2018.mat',path_data,product,winsize),'neg_stack');
    ...
    clear rho_stack pval_stack ps_stack pos_stack neg_stack
...
for dayn =[1,2,4]
    for vi =1:length(vin)
        for phn=[1 4 5 7]
            parfor ps = 0:120
                vi_daymet = vi_range{ps+1}; %(yr,ec,mk,phn)
                [rho_full, pval_full] = f_cal_partial_preseason_weight(vi_daymet, daymet_names, vi, phn, dayn, mkn, etype, 1, 19, park_veg_ratio);                        
...
save(sprintf('%s/Rho_%s_DS_mv_full_2018.mat',path_data,product),'rho_stack_full');
save(sprintf('%s/Pval_%s_DS_mv_full_2018.mat',path_data,product),'pval_stack_full');
save(sprintf('%s/PS_%s_DS_mv_full_2018.mat',path_data,product),'ps_stack_full');
