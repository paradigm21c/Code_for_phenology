    ...
    for vi =1:length(vin)
        for phn=[1 4 5 7] %:length(phenology_names)
            ...
            for mv =1:length(winsize)
                ...
                h=plot(2000+floor(winsize(mv)/2):2022-(winsize(mv)/2)+1,percent_save_pos*100);
                h.Marker = mark{mv};
                h.MarkerFaceColor = newmap(1+color_interval*(mv-1),:);
                h.MarkerEdgeColor = 'none';
                h.LineStyle = "none";
                mln= fitlm(2000+floor(winsize(mv)/2):2022-(winsize(mv)/2)+1,percent_save_pos*100);
            ...
            savefig(gcf,sprintf('%s/Plot_X_year_Y_%s_%s_tree_SigPer_pos_Fusion_%s.fig',path_figure ,phenology_names{phn},daymet_names{dayn},vin{vi}))
            saveas(gcf,sprintf('%s/Plot_X_year_Y_%s_%s_tree_SigPer_pos_Fusion_%s.png',path_figure ,phenology_names{phn},daymet_names{dayn},vin{vi}))
