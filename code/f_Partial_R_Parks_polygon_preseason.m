...
if isempty(gcp('nocreate'))
    parpool('Processes',14); % Adjust the number of workers as needed
end

product='SDC_full';
% product='MODIS';

file_path = '/gpfs/gibbs/pi/seto/jk2954/Product/Parks_polygon/ParkOG/';
...
files_mask = dir(sprintf('/home/jk2954/gibbs_pi/Product/mask/parks_arc_%s/*.tif',product));
...
if strcmp(product, 'SDC_full')
    ...
    % vp_all=importdata(sprintf('vp_merged_arc_%s.mat',product));  %(fn,ec,mk)
...
elseif strcmp(product,'MODIS')
    ...
    vp_all=importdata(sprintf('vp_all_arc_%s.mat',product));  %(fn,ec,mk)
end
...
% end
% 
% save(sprintf('%s/vi_daymet_%s.mat',file_path,product),'vi_daymet','-v7.3');

vi_daymet=importdata(sprintf('%s/vi_daymet_%s.mat',file_path,product));  
