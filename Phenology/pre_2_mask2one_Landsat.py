...
mask_files = [file for file in os.listdir(maskfile_path) if file.endswith('Landsat.tif')]
...
for mask_file in mask_files:
    with rasterio.open(os.path.join(maskfile_path, mask_file)) as src:
        ...
        if combined_mask is None:
            ...
            # Combine the masks: pixel is 1 if it's 1 in any of the masks
            combined_mask = np.maximum(combined_mask, mask_data)

# Set combined mask metadata
combined_mask[combined_mask ==np.nan] =0

# Save the combined mask as a new GeoTIFF file
combined_mask_file = os.path.join(maskfile_path, 'combined_mask_Landsat.tif')
