...
def main():
    ...
    target_directory = r'C:\SetoLab\PF\Cropped'
    masks_path = r'D:\SetoLab\Phenology\mask'
    reference_files = [f for f in os.listdir(reference_directory) if f.endswith('.tif')]
    
    for reference_file in reference_files:  
        temp_name = reference_file.replace('.tif', '')
        files_path = os.path.join(target_directory, temp_name)
    
        file_list = [file for file in os.listdir(files_path) if file.endswith('.tif')]
        mask_path_file = os.path.join(masks_path, f'combined_mask_{temp_name}.tif')  # Adjusted to use temp_name

        dates = [extract_date_from_filename(f) for f in file_list]
        years = np.array([date.year for date in dates])
        
        for year in [2019,2020,2021,2022,2023]: #np.unique(years):
            # Inside your loop
            ...
            processed_data = process_files(files_path, temp_file_list, mask)
    
            # Save processed_data to a file
            output_file_path = os.path.join(data_save_path, f'processed_data_{temp_name}_{year}.pkl')
